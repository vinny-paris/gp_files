...
make_design <- function(data_summary, name = name, it = it){
...
create_grouping_exp <- function(file = NULL, num_of_groups = 10, data = NULL, name = name, it = it){
  if(length(data) == 0 & length(file) == 0) {stop("Please pass a file or
  a dataframe to this function")}
  if(length(data) != 0 & length(file) != 0) {stop("Please pass only a single
  file or dataframe to this function")}
  
  exp_data <- make_groups(file, group_size = num_of_groups, data) 
  design_matrix <- make_design(exp_data, name = name, it = it)
...
design_with_defaults <- function(data_summary, data, sig_groups = sig_groups){
...
grouping_wrapper <- function(data, num_of_groups = 10, new_groups = 6, fractional_report = 25, groups_interest = 6, fields_interest = 6, name = name){
  ...
  exper <- create_grouping_exp(data = data, num_of_groups = num_of_groups, name = name, it = i)
  data  <- exper[[1]]
...
grouping_it <- function(data, sig_groups = NULL, num_of_groups = 10){
  ...
  j <- data$Group %in% sig_groups
  new_groupss <- subset(data, j)
...
matrix_writer <- function(mat, location){

  write.table(mat, paste(location, "matrix.csv", sep = "/"), row.names = NA, 
  col.names = NA, sep = ",")
}
...
idf_creator <- function(base_idf, param_file){
system(paste('./idf/dd -i ', base_idf, " -o ./idf/outputs -m ./idf/matrix.csv -p ", param_file, sep = " "))
}
